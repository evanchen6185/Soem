...
extern char IOmap[512];
extern uint16 cur_status;
extern int32 cur_pos;
extern uint8 cur_mode;
extern uint32_t time_s , time_us;
extern uint8 all_slave_op;
...
void ecat_loop(void)
{
    ...
    if(all_slave_op == 1)
    {
			ec_send_processdata();/* PDO发送函数 */
			ec_receive_processdata(EC_TIMEOUTRET);/* PDO接收函数 */
//				 outputs1->out1=time_cnt;	
				 for(i=0;i<ec_slavecount;i++)
				 {
//						printf("%d slaves output %d.\r\n",i+1,time_cnt); 
				    IOmap[0+i*6]=time_cnt;//此处可以给多个同样的从站IO模块输出，6为每一个模块的输出字节长度。
