...
#line 55 "..\\ETHERCAT\\ethercatmain.c"
...
#line 56 "..\\ETHERCAT\\ethercatmain.c"
...
#line 57 "..\\ETHERCAT\\ethercatmain.c"
...
#line 58 "..\\ETHERCAT\\ethercatmain.c"
...
#line 59 "..\\ETHERCAT\\ethercatmain.c"
...
#line 60 "..\\ETHERCAT\\ethercatmain.c"
...
#line 61 "..\\ETHERCAT\\ethercatmain.c"
...
static uint8            ec_esibuf[128 << 5];
 
static uint32           ec_esimap[128];
...
int ecx_init(ecx_contextt *context)
{
   return ecx_setupnic(context->port, 0);
}
...
uint8 ecx_siigetbyte(ecx_contextt *context, uint16 slave, uint16 address)
{
   ...
   {
      memset(context->esimap, 0x00, 128 * sizeof(uint32));  
   ...
   if (address < 128 << 5)
   {
      ...
      else
      {
         ...
         eadr = address >> 1;
         edat = ecx_readeepromFP (context, configadr, eadr, 20000);
          
         if (context->slavelist[slave].eep_8byte)
         {
            (memcpy((&(context->esibuf[eadr << 1])), &(edat), 8));
            cnt = 8;
         }
          
         else
         {
            (memcpy((&(context->esibuf[eadr << 1])), &(edat), 4));
            cnt = 4;
         }
...
void ecx_siistring(ecx_contextt *context, char *str, uint16 slave, uint16 Sn)
{
   ...
   {
      ba = a + 2;  
      n = ecx_siigetbyte(context, slave, ba++);  
      if (Sn <= n)  
      {
         for (i = 1; i <= Sn; i++)  
         {
            l = ecx_siigetbyte(context, slave, ba++);  
            if (i < Sn)
            {
               ba += l;
            }
...
int ecx_siiPDO(ecx_contextt *context, uint16 slave, ec_eepromPDOt* PDO, uint8 t)
{
   ...
   PDO->Length = 0;
   PDO->Index[1] = 0;
   for (c = 0 ; c < 8 ; c++) PDO->SMbitsize[c] = 0;
   if (t > 1)
      t = 1;
...
int ecx_FPRD_multi(ecx_contextt *context, int n, uint16 *configlst, ec_alstatust *slstatlst, int timeout)
{
   ...
   sldatapos[slcnt] = sizeof(ec_comt);
   while(++slcnt < (n - 1))
   {
      sldatapos[slcnt] = ecx_adddatagram(port, &(port->txbuf[idx]), EC_CMD_FPRD, idx, 1,
                            *(configlst + slcnt), ECT_REG_ALSTAT, sizeof(ec_alstatust), slstatlst + slcnt);
   }
   if(slcnt < n)
   {
      sldatapos[slcnt] = ecx_adddatagram(port, &(port->txbuf[idx]), EC_CMD_FPRD, idx, 0,
                            *(configlst + slcnt), ECT_REG_ALSTAT, sizeof(ec_alstatust), slstatlst + slcnt);
   }
...
int ecx_readstate(ecx_contextt *context)
{
   ...
   do
   {
      ...
      ecx_FPRD_multi(context, (lslave - fslave) + 1, &(slca[0]), &(sl[0]), (2000 * 3));
      for (slave = fslave; slave <= lslave; slave++)
      {
         configadr = context->slavelist[slave].configadr;
         rval = (sl[slave - fslave]. alstatus);
         context->slavelist[slave].ALstatuscode = (sl[slave - fslave]. alstatuscode);
         if (rval < lowest)
         {
            lowest = rval;
         }
...
int ecx_writestate(ecx_contextt *context, uint16 slave)
{
   ...
   if (slave == 0)
   {
      slstate = (context->slavelist[slave]. state);
      ecx_BWR(context->port, 0, ECT_REG_ALCTL, sizeof(slstate), &slstate, (2000 * 3));  
   }
   else
   {
      configadr = context->slavelist[slave].configadr;

      ecx_FPWRw(context->port, configadr, ECT_REG_ALCTL, (context->slavelist[slave]. state), (2000 * 3));  
   }
   return 0;
}
...
uint16 ecx_statecheck(ecx_contextt *context, uint16 slave, uint16 reqstate, int timeout)
{
   ...
   do
   {
      ...
      {
         rval = 0;
         ecx_BRD(context->port, 0, ECT_REG_ALSTAT, sizeof(rval), &rval , 2000);
         rval = (rval);
      }
      else
      {
         ...
         ecx_FPRD(context->port, configadr, ECT_REG_ALSTAT, sizeof(slstat), &slstat, 2000);
         rval = (slstat . alstatus);
         context->slavelist[slave].ALstatuscode = (slstat . alstatuscode);
      }
      state = rval & 0x000f;  
      if (state != reqstate)
      {
...
int ecx_mbxempty(ecx_contextt *context, uint16 slave, int timeout)
{
   ...
   {
      SMstat = 0;
      wkc = ecx_FPRD(context->port, configadr, ECT_REG_SM0STAT, sizeof(SMstat), &SMstat, 2000);
      SMstat = (SMstat);
      if (((SMstat & 0x08) != 0) && (timeout > 200))
      {
         Delay_us(200);
      }
   }
...
int ecx_mbxsend(ecx_contextt *context, uint16 slave,ec_mbxbuft *mbx, int timeout)
{
   ...
   if ((mbxl > 0) && (mbxl <= 1486))
   {
      if (ecx_mbxempty(context, slave, timeout))
      {
         ...
         wkc = ecx_FPWR(context->port, configadr, mbxwo, mbxl, mbx, (2000 * 3));
...
int ecx_mbxreceive(ecx_contextt *context, uint16 slave, ec_mbxbuft *mbx, int timeout)
{
   ...
   if ((mbxl > 0) && (mbxl <= 1486))
   {
      ...
      {
         SMstat = 0;
         wkc = ecx_FPRD(context->port, configadr, ECT_REG_SM1STAT, sizeof(SMstat), &SMstat, 2000);
				 SMstat = (SMstat);
				 hsj[9]=SMstat;
         if (((SMstat & 0x08) == 0) && (timeout > 200))
         {
      ...
      if ((wkc > 0) && ((SMstat & 0x08) > 0))  
      {
         ...
         {
					  hsj[4]++;
            wkc = ecx_FPRD(context->port, configadr, mbxro, mbxl, mbx, 2000);  
            hsj[1]=wkc;
            ...
            else if ((wkc > 0) && ((mbxh->mbxtype & 0x0f) == 0x03))  
            {
               EMp = (ec_emcyt *)mbx;
							hsj[2]=wkc;
               if (((EMp->CANOpen) >> 12) == 0x01)  
               {
                  ecx_mbxemergencyerror(context, slave, (EMp->ErrorCode), EMp->ErrorReg,
                          EMp->bData, (EMp->w1), (EMp->w2));
                  wkc = 0;  
               }
							 hsj[3]=wkc;
            }
            else
            {
               if (wkc <= 0)  
               {
                  SMstat ^= 0x0200;  
                  SMstat = (SMstat);
                  wkc2 = ecx_FPWR(context->port, configadr, ECT_REG_SM1STAT, sizeof(SMstat), &SMstat, 2000);
								 SMstat = (SMstat);
				        timer2[1]=bsp_GetTickCount();
                  do  
                  {
                     wkc2 = ecx_FPRD(context->port, configadr, ECT_REG_SM1CONTR, sizeof(SMcontr), &SMcontr, 2000);
									} while (((wkc2 <= 0) || ((SMcontr & 0x02) != (((SMstat) >> 8) & 0x02))) && (bsp_GetTickCount()-timer2[1]<timeout));
             timer2[2]=bsp_GetTickCount();
				   do  
                  {
                     wkc2 = ecx_FPRD(context->port, configadr, ECT_REG_SM1STAT, sizeof(SMstat), &SMstat, 2000);
										SMstat = (SMstat);
                     if (((SMstat & 0x08) == 0) && (timeout > 200))
                     {
                        Delay_us(200);
						...
						hsj[5]=wkc;
						hsj[6]=bsp_GetTickCount();
						hsj[7]=timer2[3];
						hsj[8]=bsp_GetTickCount()-timer2[3];
         } while ((wkc <= 0) && (bsp_GetTickCount()-timer2[3]<timeout));  
      }
      else  
      {
          wkc = 0;
...
void ecx_esidump(ecx_contextt *context, uint16 slave, uint8 *esibuf)
{
   ...
   do
   {
      edat = ecx_readeepromFP(context, configadr, address, 20000);
      p64 = (uint64*)p16;
      *p64 = edat;
      p16 += incr;
      address += incr;
   } while ((address <= (128 << 5 >> 1)) && ((uint32)edat != 0xffffffff));
...
int ecx_eeprom2master(ecx_contextt *context, uint16 slave)
{
   ...
   if ( context->slavelist[slave].eep_pdi )
   {
      ...
      do
      {
         wkc = ecx_FPWR(context->port, configadr, ECT_REG_EEPCFG, sizeof(eepctl), &eepctl , 2000);  
      }
      while ((wkc <= 0) && (cnt++ < 3));
      eepctl = 0;
      cnt = 0;
...
uint16 ecx_eeprom_waitnotbusyAP(ecx_contextt *context, uint16 aiadr,uint16 *estat, int timeout)
{
   ...
   do
   {
      ...
      *estat = 0;
      wkc=ecx_APRD(context->port, aiadr, ECT_REG_EEPSTAT, sizeof(*estat), estat, 2000);
      *estat = (*estat);
   }
   while (((wkc <= 0) || ((*estat & 0x8000) > 0)) && ((bsp_GetTickCount()-timer2)<timeout));  
   if ((*estat & 0x8000) == 0)
   {
      retval = 1;
   }
...
uint64 ecx_readeepromAP(ecx_contextt *context, uint16 aiadr, uint16 eeproma, int timeout)
{
   ...
   if (ecx_eeprom_waitnotbusyAP(context, aiadr, &estat, timeout))
   {
      ...
      {
         estat = (EC_ECMD_NOP);  
         wkc = ecx_APWR(context->port, aiadr, ECT_REG_EEPCTL, sizeof(estat), &estat, (2000 * 3));
      }

      do
      {
         ed.comm = (EC_ECMD_READ);
         ed.addr = (eeproma);
         ed.d2   = 0x0000;
         cnt = 0;
         do
         {
            wkc = ecx_APWR(context->port, aiadr, ECT_REG_EEPCTL, sizeof(ed), &ed, 2000);
         }
         while ((wkc <= 0) && (cnt++ < 3));
         if (wkc)
         {
            ...
            if (ecx_eeprom_waitnotbusyAP(context, aiadr, &estat, timeout))
            {
               ...
               else
               {
                  ...
                  if (estat & 0x0040)
                  {
                     ...
                     do
                     {
                        wkc = ecx_APRD(context->port, aiadr, ECT_REG_EEPDAT, sizeof(edat64), &edat64, 2000);
                     }
                     while ((wkc <= 0) && (cnt++ < 3));
                  }
                  else
                  {
                     ...
                     do
                     {
                        wkc = ecx_APRD(context->port, aiadr, ECT_REG_EEPDAT, sizeof(edat32), &edat32, 2000);
                     }
                     while ((wkc <= 0) && (cnt++ < 3));
...
int ecx_writeeepromAP(ecx_contextt *context, uint16 aiadr, uint16 eeproma, uint16 data, int timeout)
{
   ...
   if (ecx_eeprom_waitnotbusyAP(context, aiadr, &estat, timeout))
   {
      ...
      do
      {
         ...
         do
         {
            wkc = ecx_APWR(context->port, aiadr, ECT_REG_EEPDAT, sizeof(data), &data, 2000);
         }
         while ((wkc <= 0) && (cnt++ < 3));
...
uint16 ecx_eeprom_waitnotbusyFP(ecx_contextt *context, uint16 configadr,uint16 *estat, int timeout)
{
   ...
   do
   {
      ...
      *estat = 0;
      wkc=ecx_FPRD(context->port, configadr, ECT_REG_EEPSTAT, sizeof(*estat), estat, 2000);
      *estat = (*estat);
   }
   while (((wkc <= 0) || ((*estat & 0x8000) > 0)) && ((bsp_GetTickCount()-timer2)<timeout));  
   if ((*estat & 0x8000) == 0)
...
uint64 ecx_readeepromFP(ecx_contextt *context, uint16 configadr, uint16 eeproma, int timeout)
{
   ...
   if (ecx_eeprom_waitnotbusyFP(context, configadr, &estat, timeout))
   {
      ...
      {
         estat = (EC_ECMD_NOP);  
         wkc=ecx_FPWR(context->port, configadr, ECT_REG_EEPCTL, sizeof(estat), &estat, (2000 * 3));
      }
      ...
      do
      {
         ...
         do
         {
            wkc=ecx_FPWR(context->port, configadr, ECT_REG_EEPCTL, sizeof(ed), &ed, 2000);
         }
         while ((wkc <= 0) && (cnt++ < 3));
         if (wkc)
         {
            ...
            if (ecx_eeprom_waitnotbusyFP(context, configadr, &estat, timeout))
            {
               ...
               else
               {
                  ...
                  if (estat & 0x0040)
                  {
                     ...
                     do
                     {
                        wkc=ecx_FPRD(context->port, configadr, ECT_REG_EEPDAT, sizeof(edat64), &edat64, 2000);
                     }
                     while ((wkc <= 0) && (cnt++ < 3));
                  }
                  else
                  {
                     ...
                     do
                     {
                        wkc=ecx_FPRD(context->port, configadr, ECT_REG_EEPDAT, sizeof(edat32), &edat32, 2000);
                     }
                     while ((wkc <= 0) && (cnt++ < 3));
                     edat64=(uint64)edat32;
                  }
...
int ecx_writeeepromFP(ecx_contextt *context, uint16 configadr, uint16 eeproma, uint16 data, int timeout)
{
   ...
   if (ecx_eeprom_waitnotbusyFP(context, configadr, &estat, timeout))
   {
      ...
      do
      {
         ...
         do
         {
            wkc = ecx_FPWR(context->port, configadr, ECT_REG_EEPDAT, sizeof(data), &data, 2000);
         }
         while ((wkc <= 0) && (cnt++ < 3));
         ed.comm = EC_ECMD_WRITE;
         ed.addr = eeproma;
...
void ecx_readeeprom1(ecx_contextt *context, uint16 slave, uint16 eeproma)
{
   ...
   ecx_eeprom2master(context, slave);  
   configadr = context->slavelist[slave].configadr;
   if (ecx_eeprom_waitnotbusyFP(context, configadr, &estat, 20000))
   {
      if (estat & 0x7800)  
      {
         estat = (EC_ECMD_NOP);  
...
uint32 ecx_readeeprom2(ecx_contextt *context, uint16 slave, int timeout)
{
   ...
   if (ecx_eeprom_waitnotbusyFP(context, configadr, &estat, timeout))
   {
      do
      {
          wkc = ecx_FPRD(context->port, configadr, ECT_REG_EEPDAT, sizeof(edat), &edat, 2000);
      }
      while ((wkc <= 0) && (cnt++ < 3));
   }
