...
static unsigned char DEF_MAC_ADDR[6] = {0x01, 0x02, 0x01, 0x02, 0x01, 0x02};
uint8_t SendPackOk = 0;
uint8_t s_FSMC_Init_Ok = 0;	 

u16  receiveLen_DM9000;
u8   receiveBuffer_DM9000[1600];
...
static void DM9K_CtrlLinesConfig(void)
{
  ...
  NVIC_InitTypeDef NVIC_InitStructure;
	...
	RCC_AHB1PeriphClockCmd(((uint32_t)0x00000001)|((uint32_t)0x00000008) | ((uint32_t)0x00000010) | ((uint32_t)0x00000040), ENABLE);
  RCC_APB2PeriphClockCmd(((uint32_t)0x00004000), ENABLE);
	...
	GPIO_InitStructure.GPIO_Pin = ((uint16_t)0x0010) | ((uint16_t)0x0020) | ((uint16_t)0x0080) | ((uint16_t)0x0100) | ((uint16_t)0x0200) | ((uint16_t)0x0400) |
  ...
  SYSCFG_EXTILineConfig(((uint8_t)0x00), ((uint8_t)0x0F));

   
  EXTI_InitStructure.EXTI_Line = ((uint32_t)0x08000);
...
{
	(*((volatile uint16_t *) 0x68400000)) = reg;
	return ((*((volatile uint16_t *) (0x68400000 + 0x00080000))));
}
...
{
	(*((volatile uint16_t *) 0x68400000)) = reg;
	(*((volatile uint16_t *) (0x68400000 + 0x00080000))) = writedata;
}
...
void dm9k_hash_table(void)
{
	...
	for(i = 0; i < 6; i++)
	{
		dm9k_WriteReg(0x10 + i, DEF_MAC_ADDR[i]);
	}

	 
	for(i = 0; i < 8; i++)
	{
		dm9k_WriteReg(0x16 + i, 0xFF);
	}
...
void dm9k_reset(void)
{
 	...
 	dm9k_WriteReg(0x1E,0x01);			
	dm9k_WriteReg(0x1F,0);				
	Delay_ms(2);                          
 	dm9k_WriteReg(0x00,0x03);	   
	Delay_us(20);                        
	dm9k_WriteReg(0x00,0x00);	   
	dm9k_WriteReg(0x00,0x03);	   
	Delay_us(20);                        
	dm9k_WriteReg(0x00,0x00);	   
		 
	dm9k_WriteReg(0xFF, 0x80); 			 
	dm9k_WriteReg(0x2D, 0x80);			 
	 
	dm9k_WriteReg(0x01, 0x2c);
	dm9k_WriteReg(0x02, 0x00);
	dm9k_WriteReg(0xFE, 0x3f);
	dm9k_WriteReg(0x05, 0x33);			 
	dm9k_WriteReg(0xFF, 0x81);			 
	SendPackOk = 0;
}
...
void dm9k_phy_write(uint8_t phy_reg, uint16_t writedata)
{
	...
	dm9k_WriteReg(0x0C, phy_reg | 0x40);

	 
	dm9k_WriteReg(0x0E, ( writedata >> 8 ) & 0xff);
	dm9k_WriteReg(0x0D, writedata & 0xff);

	dm9k_WriteReg(0x0B, 0x0a); 						 

	Delay_ms(50);

	 dm9k_WriteReg(0x0B, 0x00); 	
}
...
void dm9k_initnic(void)
{
	...
	dm9k_phy_write(0x00, 0x1000);				 
	dm9k_WriteReg(0x1F, 0x00);				 
	dm9k_hash_table();								 
...
uint16_t dm9k_receive_packet(void)
{
	...
	{
		jump_packet = 0;								 
		  temp=dm9k_ReadReg(0xF5);
   temp=dm9k_ReadReg(0xF4);
		dm9k_ReadReg(0xF0);			 
		rx_checkbyte = dm9k_ReadReg(0xF0);			 
		 
		calc_MRR = (dm9k_ReadReg(0xF5) << 8) + dm9k_ReadReg(0xF4);
		hreh=rx_checkbyte;
		if(rx_checkbyte == 0x01)				 
		{
			 
			(*((volatile uint16_t *) 0x68400000)) = 0xF2;
			rx_status = (*((volatile uint16_t *) (0x68400000 + 0x00080000)));
			rx_length = (*((volatile uint16_t *) (0x68400000 + 0x00080000)));
			...
			if(jump_packet == 0x01)
			{
				...
				dm9k_WriteReg (0xF5, (calc_MRR >> 8) & 0xff);
				dm9k_WriteReg (0xF4, calc_MRR & 0xff );
				continue;
			}
       receiveLen_DM9000 = 0;								 
			...
			for(i = 0 ; i < calc_len ; i++)
			  ReceiveData[i]= (*((volatile uint16_t *) (0x68400000 + 0x00080000)));
      ...
      memcpy((unsigned char*)receiveBuffer_DM9000,(uint8_t *)ReceiveData,receiveLen_DM9000);
		...
		{
			if(rx_checkbyte == 0x00)		 
			{
				dm9k_WriteReg(0xFE,0X02);			
        dm9k_WriteReg(0xFF, 0x81);				
			}
			else
			{
...
void dm9k_send_packet(uint8_t *p_char, uint16_t length)
{
	...
	uint16_t i;
	uint16_t calc_len;
	volatile uint16_t calc_MWR;

	 
	dm9k_WriteReg(0xFF,0x80);		
	if(SendPackOk == 2)
	{
		while(dm9k_ReadReg(0x02) & 0x01)
		{
			Delay_us (5);
		}
		SendPackOk = 0;
...
#line 607 "..\\HARDWARE\\dm9k_uip.c"

	dm9k_WriteReg(0xFD, (SendLength >> 8) & 0xff);	 
	dm9k_WriteReg(0xFC, SendLength & 0xff);

	 
	(*((volatile uint16_t *) 0x68400000)) = 0xF8;
	calc_len = (SendLength + 1) >> 1;
	for(i = 0; i < calc_len; i++)
		(*((volatile uint16_t *) (0x68400000 + 0x00080000))) = SendData[i];

	dm9k_WriteReg(0x02, 0x01);				 


  while(dm9k_ReadReg(0x01)&0X2c);			
	dm9k_WriteReg(0x01, 0x2c);
 	dm9k_WriteReg(0xFF,0x81);			
	
  sendk++;
#line 639 "..\\HARDWARE\\dm9k_uip.c"
	return;
}
...
void  dm9k_interrupt(void)
{
	...
	save_reg = (*((volatile uint16_t *) 0x68400000));							 
  hahks++;
	dm9k_WriteReg(0xFF , 0x80);				 
	isr_status = dm9k_ReadReg(0xFE);					 
	...
	dm9k_WriteReg(0xFF , 0x81);				 
	(*((volatile uint16_t *) 0x68400000)) = save_reg;							 
...
void EXTI15_10_IRQHandler(void)
{
	...
	if(EXTI_GetITStatus(((uint32_t)0x08000)) != RESET)
  {
    ...
    EXTI_ClearITPendingBit(((uint32_t)0x08000));
  }
}
...
uint32_t dm9k_ReadID(void)
{
	...
	vid1 = dm9k_ReadReg(0x28) & 0xFF;
	vid2 = dm9k_ReadReg(0x29) & 0xFF;
	pid1 = dm9k_ReadReg(0x2A) & 0xFF;
	pid2 = dm9k_ReadReg(0x2B) & 0xFF;
