...
const uint16 priMAC[3] = { 0x0102, 0x0102, 0x0102 };
...
extern uint16  receiveLen;
extern uint8 receiveBuffer[];
...
void ec_setupheader(void *p)
{
   ...
   bp->da0 = ((((uint16)(0xffff) & 0xff00) >> 8) | (((uint16)(0xffff) & 0x00ff) << 8));
   bp->da1 = ((((uint16)(0xffff) & 0xff00) >> 8) | (((uint16)(0xffff) & 0x00ff) << 8));
   bp->da2 = ((((uint16)(0xffff) & 0xff00) >> 8) | (((uint16)(0xffff) & 0x00ff) << 8));
   bp->sa0 = ((((uint16)(priMAC[0]) & 0xff00) >> 8) | (((uint16)(priMAC[0]) & 0x00ff) << 8));
   bp->sa1 = ((((uint16)(priMAC[1]) & 0xff00) >> 8) | (((uint16)(priMAC[1]) & 0x00ff) << 8));
   bp->sa2 = ((((uint16)(priMAC[2]) & 0xff00) >> 8) | (((uint16)(priMAC[2]) & 0x00ff) << 8));
   bp->etype = ((((uint16)(0x88A4) & 0xff00) >> 8) | (((uint16)(0x88A4) & 0x00ff) << 8));
}
...
int ecx_getindex(ecx_portt *port)
{
   ...
   while ((port->rxbufstat[idx] != EC_BUF_EMPTY) && (cnt < 2))
   {
      idx++;
      cnt++;
      if (idx >= 2)
...
int ecx_outframe(ecx_portt *port, int idx, int stacknumber)
{
   ...
   lp = (*stack->txbuflength)[idx];
	 DM9161_EthStatus=ETH_ReadPHYRegister(((uint16_t) 0x01), 1) & 0x0004;
	 DM9000_EthStatus=(dm9k_ReadReg(0x01) & 0x40);
	 switch(Switch_Net)
	   {
		 case 0:
			 if(DM9161_EthStatus)
		 ...
		 case 3:
			  if((DM9161_EthStatus)&&(DM9000_EthStatus)) 
				 {
					 if((DM9161_Send_Receive==1)&&(DM9000_Send_Receive==0))
					    MAC_Send((*stack->txbuf)[idx], lp);
					 if((DM9161_Send_Receive==0)&&(DM9000_Send_Receive==1))
				     dm9k_send_packet((*stack->txbuf)[idx], lp);
				 }
				 else
				   return 0;
...
int ecx_outframe_red(ecx_portt *port, int idx)
{
   ...
   ehp->sa1 = ((((uint16)(priMAC[1]) & 0xff00) >> 8) | (((uint16)(priMAC[1]) & 0x00ff) << 8));
...
int ecx_inframe(ecx_portt *port, int idx, int stacknumber)
{
   ...
   if ((idx < 2) && ((*stack->rxbufstat)[idx] == EC_BUF_RCVD)) 
   {
   ...
   else 
   {
      ...
      if (ecx_recvpkt(port, stacknumber)) 
      {
         ...
         if (ehp->etype == ((((uint16)(0x88A4) & 0xff00) >> 8) | (((uint16)(0x88A4) & 0x00ff) << 8))) 
         {
            ecp =(ec_comt*)(&(*stack->tempbuf)[sizeof(ec_etherheadert)]); 
            l = (ecp->elength) & 0x0fff;
            ...
            if (idxf == idx) 
            {
               ...
               memcpy(rxbuf, &(*stack->tempbuf)[sizeof(ec_etherheadert)], (*stack->txbuflength)[idx] - sizeof(ec_etherheadert));
							 
							 l=(*stack->txbuflength)[idx] - sizeof(ec_etherheadert);
               rval = ((*rxbuf)[l-2] + ((uint16)((*rxbuf)[l-1]) << 8));
                
               (*stack->rxbufstat)[idx] = EC_BUF_COMPLETE;
                
               (*stack->rxsa)[idx] = (ehp->sa1);
            }
            else 
            {
               ...
               if (idxf < 2) 
               {
                  ...
                  memcpy(rxbuf, &(*stack->tempbuf)[sizeof(ec_etherheadert)], (*stack->txbuflength)[idxf] - sizeof(ec_etherheadert));
                   
                  (*stack->rxbufstat)[idxf] = EC_BUF_RCVD;
                  (*stack->rxsa)[idxf] = (ehp->sa1);
               }
               else 
               {
...
uint16_t hkll;
static int ecx_waitinframe_red(ecx_portt *port, int idx, int timeout)
{
   int wkc  = -1;
   uint32_t timer2;
   ...
   } while ((wkc <= -1) &&((bsp_GetTickCount()-timer2)<timeout));
